...
node = QualibrationNode(name="05c_T2_CPMG", parameters=Parameters())
...
idle_times = np.arange(
    ...
    node.parameters.max_wait_time_in_ns // node.parameters.N // 8,
...
def play_N_Pi(xy, t, n_pi):
    if n_pi == 0:
        return
    xy.play('x180')
    for i in range(1,n_pi):
        xy.wait(2*t)
        xy.play('x180')
    return
    ...
    for i, qubit in enumerate(qubits):
        ...
        with for_(shot[i], 0, shot[i] < n_avg, shot[i] + 1):
            ...
            with for_(*from_array(t[i], idle_times)):
                ...
                qubit.xy.play("x90")
                qubit.xy.wait(t[i])
                play_N_Pi(qubit.xy,t[i],node.parameters.N)
                qubit.xy.wait(t[i])
                qubit.xy.play("x90")
                if node.parameters.N % 2 == 1:
                    qubit.xy.play("x180")
                qubit.align()
...
if not node.parameters.simulate:
    if node.parameters.load_data_id is None:
        ...
        ds = ds.assign_coords(idle_time=8 * node.parameters.N * ds.idle_time / u.us)  # convert to µs
    ...
    grid.fig.suptitle("T2 (N = {:d})".format(node.parameters.N))
    plt.tight_layout()
    plt.show()
