...
class Parameters(NodeParameters):
    ...
    operation_EF_x180_or_any_90: Literal["EF_x180", "EF_x90", "-EF_x90", "EF_y90", "-EF_y90","x180", "x90", "-x90", "y90", "-y90"] = "EF_x180"
    enable_x180_pre: bool = True
    enable_x180_post: bool = False
    min_amp_factor: float = 0.0
    max_amp_factor: float = 3.0
    amp_factor_step: float = 0.01
    frequency_offset_in_mhz: float = 0.0
    ...
    state_discrimination_thresh_1mV: float = 0.5
...
node = QualibrationNode(name="04b_Power_Rabi_ef_3d", description=description, parameters=Parameters())
...
state_discrimination_thresh_1mV = node.parameters.state_discrimination_thresh_1mV
operation = node.parameters.operation_EF_x180_or_any_90  # The qubit operation to play
...
dfs = np.arange(-span // 2 + node.parameters.frequency_offset_in_mhz * u.MHz, span // 2 + node.parameters.frequency_offset_in_mhz * u.MHz, step, dtype=np.int32)
...
if N_pi > 1:
    ...
    elif operation in ["EF_x90", "-EF_x90", "EF_y90", "-EF_y90","x90", "-x90", "y90", "-y90"]:
...
with program() as power_rabi:
    ...
    df = [declare(int) for _ in range(num_qubits)]  # QUA variable for the readout frequency
                ...
                with for_(*from_array(df[i], dfs)):
                    with for_(*from_array(a[i], amps)):
                        ...
                        if node.parameters.enable_x180_pre:
                            qubit.xy.play("x180")  # Initialize qubit in |1> state
                            qubit.align()
                        ...
                        (
                            qubit.xy.name, 
                            qubit.xy.intermediate_frequency - qubit.anharmonicity + df[i]
                        )
        ...
        for i, qubit in enumerate(qubits):
            if operation in ["EF_x180","x180"]:
                if state_discrimination:
                    ...
                    I_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(np.ceil(N_pi / 2)).save(f"I{i + 1}")
                    Q_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(np.ceil(N_pi / 2)).save(f"Q{i + 1}")

            elif operation in ["EF_x90", "-EF_x90", "EF_y90", "-EF_y90","x90", "-x90", "y90", "-y90"]:
                if state_discrimination:
                    state_stream[i].boolean_to_int().buffer(len(amps)).buffer(len(dfs)).average().buffer(np.ceil(N_pi / 4)).average().save(
                        f"state{i + 1}"
                    )
                else:
                    I_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(np.ceil(N_pi / 4)).save(f"I{i + 1}")
                    Q_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(np.ceil(N_pi / 4)).save(f"Q{i + 1}")
            else:
                raise ValueError(f"Unrecognized operation {operation}.")
...
if not node.parameters.simulate:
        ...
        ds = ds.assign_coords(
            {
                "freq_full": (
                    ["qubit", "freq"],
                    np.array([dfs + q.xy.RF_frequency - qubit.anharmonicity for q in qubits]),
                )
            }
        )
    ...
    if 0:  
        ...
        if N_pi == 1:
            ...
            if state_discrimination:
                ...
                I_n = ds.I.mean(dim="N")
            if (N_pi_vec[0] % 2 == 0 and operation == "EF_x180") or (N_pi_vec[0] % 2 != 0 and operation != "EF_x180"):
    ...
    if 0:
        ...
        for ax, qubit in grid_iter(grid):
            ...
            ds.assign_coords(freq_GHz=ds.freq_full / 1e9).loc[qubit].I.plot(
                ax=ax, add_colorbar=False, x="amp", y="freq_GHz", robust=True
            )
            ...
            ax.set_xlabel("Amplitude [mV]")
            ax.set_title(qubit["qubit"])
        grid.fig.suptitle("Rabi amp vs detuning ")
    ...
    if node.parameters.load_data_id is None:
        ...
        #         if operation == "EF_x180" and node.parameters.update_x90:
        #             q.xy.operations["EF_x90"].amplitude = fit_results[q.name]["Pi_amplitude"] / 2
