...
node = QualibrationNode(name="07c_IQ_Blobs_state_2", description=description, parameters=Parameters())
...
# # Use a QUA integer array for the frequency offsets.
# # NOTE: `declare` must be called inside the `with program() as iq_blobs:` block.
# # Replace/insert the following inside the QUA program to create a QUA-type list:
# #
# freq_range = declare(int, size=n_runs)
# idx = declare(int)
# with for_(idx, 0, idx < n_runs, idx + 1):
#     assign(freq_range[idx], int(-50e6 + idx * (100e6 // (n_runs - 1))))

# # (Kept here as documentation; the actual `declare`/`assign` must be inside the QUA program.)
        ...
        for I_base, Q_base, label_base, color in pairs:
            ...
            for I_key in I_candidates:
                ...
                ax.plot(
                    ...
                    1e3 * Q_series,
                    ".",
                    alpha=0.25 if label_base != "|2>" else .25,
                    markersize=1.5,
                    color=color,
