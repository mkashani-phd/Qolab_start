...
250-275s for all qubits
35-45s per qubit
"""

node = QualibrationNode(name="07d_state_2_Readout_Power_Optimization", description=description, parameters=Parameters())
...
with program() as iq_blobs:
    I_g, I_g_st, Q_g, Q_g_st, _, n_st = qua_declaration(num_qubits=num_qubits)
    I_e, I_e_st, Q_e, Q_e_st, _, _ = qua_declaration(num_qubits=num_qubits)
    shot = [declare(int) for _ in range(num_qubits)]
    a = [declare(fixed) for _ in range(num_qubits)]
    ...
    for i, qubit in enumerate(qubits):
        ...
        with for_(shot[i], 0, shot[i] < n_runs, shot[i] + 1):
            ...
            with for_(*from_array(a[i], amps)):
                ...
                qubit.align()
                qubit.resonator.measure("readout", qua_vars=(I_g[i], Q_g[i]), amplitude_scale=a[i])
                qubit.align()
                # save data
                save(I_g[i], I_g_st[i])
                save(Q_g[i], Q_g_st[i])
                ...
                qubit.align()
                qubit.resonator.measure("readout", qua_vars=(I_e[i], Q_e[i]), amplitude_scale=a[i])
                save(I_e[i], I_e_st[i])
                save(Q_e[i], Q_e_st[i])
