...
Modification of Qubit Spectroscopy vs. Flux that first excites the qubit to the |1> state in order
to identify both the 0-1 and 1-2 transition lines.

Prerequisites:
    - All prerequisites for 03c_Qubit_ef_Spectroscopy
    - Calibration of x180 gate
"""
...
class Parameters(NodeParameters):
    ...
    operation_amplitude_factor: Optional[float] = 0.8  # Need high amplitude to see it; Don't need to apply x180 to see it.
    operation_len_in_ns: Optional[int] = None
    enable_x180: bool = False  # Set to True if you're trying to see 1-2 transition line.
...
300-315s for all qubits
50-60s per qubit
"""


node = QualibrationNode(name="03e_Qubit_ef_Spectroscopy_vs_Flux", description=description, parameters=Parameters())
...
with program() as multi_qubit_spec_vs_flux:
    ...
    dc = [declare(fixed) for _ in range(num_qubits)]  # QUA variable for the flux dc level
    ...
    for i, qubit in enumerate(qubits):
        ...
        with for_(shot[i], 0, shot[i] < n_avg, shot[i] + 1):
            ...
            with for_(*from_array(df[i], dfs)):
                ...
                qubit.xy.update_frequency(df[i] + qubit.xy.intermediate_frequency)
                with for_(*from_array(dc[i], dcs)):
                    ...
                    qubit.z.play("const", amplitude_scale=dc[i] / qubit.z.operations["const"].amplitude, duration=duration+100)
                    # Apply saturation pulse to all qubits (saturation populates the resonator)
                    ...
                    if node.parameters.enable_x180:
                        qubit.xy.play("x180")
                        qubit.align()
    ...
    if node.parameters.load_data_id is None:
        ...
        for q in qubits:
            ...
            if not np.isnan(flux_shift.sel(qubit=q.name).values):
                ...
                fit_results[q.name]["coeff"] = coeff.data[:,0]  # Size (3,1) => (3,) for flat list
            else:
                ...
                fit_results[q.name]["coeff"] = np.array([np.nan,np.nan,np.nan])
        ...
        with node.record_state_updates():
            for q in qubits:
                if not np.isnan(flux_shift.sel(qubit=q.name).values):
                    ...
                    q.flux_vs_freq_curve = fit_results[q.name]["coeff"].tolist()
