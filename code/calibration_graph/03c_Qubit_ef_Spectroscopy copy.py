...
Prerequisites:
    - Calibration of ge transition (1-2 transition) frequency and flux, e.g. having run 03a_Qubit_Spectroscopy.py and 
      03b_qubit_spectroscopy_vs_flux.py and applied the configuration updates.
    - Edit "anharmonicity" parameter in state.json to change the center of the frequency sweep. 200000000 is a good initial guess.

Before proceeding to the next node:
    - Update the anharmonicity in the state, as well as the expected EF_x180 amplitude and IQ rotation angle.
...
class Parameters(NodeParameters):
    ...
    operation_amplitude_factor: Optional[float] = 0.8
    operation_len_in_ns: Optional[int] = None
    enable_x180_pre: bool = True
    enable_x180_post: bool = False
    frequency_offset_in_mhz: float = 0.0   # Relative to 1-2 transition (qubit frequency minus anharmonicity)
...
node = QualibrationNode(name="03c_Qubit_ef_Spectroscopy", description=description, parameters=Parameters())
...
for (q,k) in zip(qubits,detunings.keys()):
    detunings[k] = detunings[k] - q.anharmonicity
...
if not node.parameters.simulate:
    ...
    for q in qubits:
        ...
        if not np.isnan(result.sel(qubit=q.name).position.values):
            ...
            # Debug: export some additional values
            fit_results[q.name]["drive_freq_01"] = q.xy.RF_frequency
            fit_results[q.name]["intermediate_frequency"] = q.xy.intermediate_frequency
            fit_results[q.name]["postition_values"] = result.sel(qubit=q.name).position.values
            fit_results[q.name]["anharmonicity_old"] = q.anharmonicity
            fit_results[q.name]["anharmonicity_new"] = fit_results[q.name]["drive_freq_01"] - fit_results[q.name]["drive_freq"]
            #q.anharmonicity + q.xy.intermediate_frequency - result.sel(qubit=q.name).position.values
        ...
        with node.record_state_updates():
            for q in qubits:
                if not np.isnan(result.sel(qubit=q.name).position.values):
                    ...
                    # else:
                        q.anharmonicity = node.results["fit_results"][q.name]["anharmonicity_new"]
                    # if not flux_point == "arbitrary":
                        ...
                        # ) % (2 * np.pi)
                        Pi_length = q.xy.operations["EF_x180"].length
                        used_amp = q.xy.operations["saturation"].amplitude * operation_amp
                        factor_cw = float(target_peak_width / result.sel(qubit=q.name).width.values)
                        factor_pi = np.pi / (result.sel(qubit=q.name).width.values * Pi_length * 1e-9)
                        ...
                        if factor_pi * used_amp < limits.max_x180_wf_amplitude:
                            q.xy.operations["EF_x180"].amplitude = factor_pi * used_amp
                        elif factor_pi * used_amp >= limits.max_x180_wf_amplitude:
                            q.xy.operations["EF_x180"].amplitude = limits.max_x180_wf_amplitude
        node.results["ds"] = ds
