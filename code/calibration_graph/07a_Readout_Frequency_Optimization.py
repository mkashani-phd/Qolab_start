...
50-60s for all qubits
10-15s per qubit
"""

node = QualibrationNode(name="07a_Readout_Frequency_Optimization", description=description, parameters=Parameters())
    ...
    for i, qubit in enumerate(qubits):
        ...
        with for_(shot[i], 0, shot[i] < n_avg, shot[i] + 1):
            ...
            with for_(*from_array(df[i], dfs)):
                ...
                update_frequency(qubit.resonator.name, df[i] + qubit.resonator.intermediate_frequency)
