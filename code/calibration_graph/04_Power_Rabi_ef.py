...
class Parameters(NodeParameters):
    ...
    operation_EF_x180_or_any_90: Literal["EF_x180", "EF_x90", "-EF_x90", "EF_y90", "-EF_y90"] = "EF_x180"
...
node = QualibrationNode(name="04_Power_Rabi_ef", description=description, parameters=Parameters())
...
if N_pi > 1:
    ...
    elif operation in ["EF_x90", "-EF_x90", "EF_y90", "-EF_y90"]:
...
if not node.parameters.simulate:
    if node.parameters.load_data_id is None:
        ...
        if not state_discrimination:
            ...
            # Compute magnitude of IQ to use both channels
            ds = ds.assign(mag=(ds.I ** 2 + ds.Q ** 2) ** 0.5)
    ...
    if N_pi == 1:
        ...
        if state_discrimination:
            ...
            # Choose between IQ magnitude and I-only fitting
            if node.parameters.use_IQ_magnitude:
                # Fit the oscillation using the IQ magnitude for better SNR
                fit = fit_oscillation(ds.mag, "amp")
            else:
                # Fit the oscillation using only the I quadrature
    ...
    for ax, qubit in grid_iter(grid):
        if N_pi == 1:
            if state_discrimination:
                ...
                ds_plot = ds.assign_coords(amp_mV=ds.abs_amp * 1e3).loc[qubit]
                if node.parameters.use_IQ_magnitude:
                    # Plot IQ magnitude vs amplitude
                    (ds_plot.mag * 1e3).plot(ax=ax, x="amp_mV")
                    ax.set_ylabel("Trans. amp. |IQ| [mV]")
                else:
                    # Plot I quadrature vs amplitude
                    (ds_plot.I * 1e3).plot(ax=ax, x="amp_mV")
            ...
            if state_discrimination:
                ...
                ds_plot = ds.assign_coords(amp_mV=ds.abs_amp * 1e3).loc[qubit]
                if node.parameters.use_IQ_magnitude:
                    # Plot IQ magnitude vs amplitude and number of pulses
                    (ds_plot.mag * 1e3).plot(ax=ax, x="amp_mV", y="N")
                else:
                    # Plot I quadrature vs amplitude and number of pulses
                    (ds_plot.I * 1e3).plot(ax=ax, x="amp_mV", y="N")
            ax.set_ylabel("num. of pulses")
            ax.axvline(1e3 * ds.abs_amp.loc[qubit][data_max_idx.loc[qubit]], color="r")
        ax.set_xlabel("Amplitude [mV]")
        ax.set_title(qubit["qubit"])
    grid.fig.suptitle("Rabi : signal vs. amplitude")
    plt.tight_layout()
    plt.show()
