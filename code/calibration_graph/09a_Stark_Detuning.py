...
with program() as stark_detuning:
    ...
    df = [declare(int) for _ in range(num_qubits)]  # QUA variable for the qubit drive amplitude pre-factor
        ...
        with for_(shots[i], 0, shots[i] < n_avg, shots[i] + 1):
            ...
            with for_(*from_array(npi[i], N_pi_vec)):
                with for_(*from_array(df[i], dfs)):
                    ...
                    update_frequency(qubit.xy.name, df[i] + qubit.xy.intermediate_frequency)
