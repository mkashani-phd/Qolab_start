...
node = QualibrationNode(name="02b_pulsed_Resonator_Spectroscopy_vs_Flux", parameters=Parameters())
        ...
        with for_(n, 0, n < n_avg, n + 1):
            ...
            with for_(*from_array(dc, dcs)):
                with for_(*from_array(df, dfs)):
                    ...
                    qubit.align()
                    wait(20, qubit.resonator.name)
                    qubit.z.play("const", amplitude_scale=dc / qubit.z.operations["const"].amplitude, duration=rr.operations["readout"].length+40)
...
if not node.parameters.simulate:
    ...
    grid.fig.suptitle("Resonator spectroscopy vs flux pulsed")
    plt.tight_layout()
    plt.show()
