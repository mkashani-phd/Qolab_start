...
class QuAM(QuamRoot):
    ...
    def load(cls, *args, **kwargs) -> "QuAM":
        if not args:
            ...
            with open(os.path.expanduser('~/.qualibrate/config.toml'),'rb') as f:
                qualibrateconfig = tomllib.load(f)
            qualibrateconfigquamstatepath = qualibrateconfig['quam']['state_path']
