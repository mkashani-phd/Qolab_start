...
* __make_wiring__: create the port mapping between the control hardware (OPX+, Octave, OPX1000 LF fem, MW fem) and the quantum elements (qubits, resonators, flux lines...).
  * [make_wiring_lffem_mwfem.py](../../code/configuration/make_wiring_lffem_mwfem.py) for a cluster made of LF and MW FEMs (OPX1000).
  * [make_wiring_lffem_octave.py](../../code/configuration/make_wiring_lffem_octave.py) for a cluster made of LF-FEMs and Octaves (OPX1000).
  * [make_wiring_opxp_octave.py](../../code/configuration/make_wiring_opxp_octave.py) for a cluster made of OPX+ and Octaves.
* [make_quam.py](../../code/configuration/make_quam.py): create the state of the system based on the generated wiring and QUAM components and containing all the information necessary to calibrate the chip and run experiments. This state is used to generate the OPX configuration.
* [modify_quam.py](../../code/configuration/modify_quam.py): update the parameters of the state programmatically based on defaults values (previous calibration, chip manufacturer specification...).
...
### [2. The QuAM components](pkg-quam_libs/quam_libs/components)
...
1. [quam_root.py](pkg-quam_libs/quam_libs/components/quam_root.py) represents the highest level in terms of hierarchy.
...
2. The definition of a QuAM transmon is defined in [transmon.py](pkg-quam_libs/quam_libs/components/transmon.py). It contains the
   ...
   [transmon_pair.py](pkg-quam_libs/quam_libs/components/transmon_pair.py).
3. The QuAM components are either defined from the base QUAM components directly, such as the qubit xy drive which is
   ...
   [readout_resonator](pkg-quam_libs/quam_libs/components/readout_resonator.py) or [flux_line](pkg-quam_libs/quam_libs/components/flux_line.py),
...
under the [quam_builder](pkg-quam_libs/quam_libs/quam_builder) folder.
...
Note that the set of default pulses, e.g. CosineDrag and Square, can be edited in [pulses.py](pkg-quam_libs/quam_libs/quam_builder/pulses.py).
