...
if not node.parameters.simulate and node.parameters.reset_filters:
    ...
    try:
        p0 = [final_vals, -1+first_vals/final_vals, 5]
    ...
    if plot_process:
        ...
        ax.set_ylim([final_vals*0.,final_vals*1.1])
        ax.legend()
        plt.show()
...
if not node.parameters.simulate and node.parameters.reset_filters:
    ...
    if plot_process:
        ...
        # plt.ylim([final_vals*0.9,final_vals*1.1])
        plt.legend()
        plt.show()
...
if not node.parameters.simulate and node.parameters.reset_filters:
    ...
    if np.max(np.abs(convolved_fir)) > 2:
    ...
    if plot_process:
        ...
        plt.axhline(final_vals*1.001, color = 'k')
        plt.axhline(final_vals*0.999, color = 'k')
        plt.ylim([final_vals*0.6,final_vals*1.05])
        plt.legend()
        plt.show()
