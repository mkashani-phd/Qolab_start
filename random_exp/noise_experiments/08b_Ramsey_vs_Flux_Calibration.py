    ...
    for i, qubit in enumerate(qubits):
        ...
        with for_(shots[i], 0, shots[i] < n_avg, shots[i] + 1):
            ...
            with for_(flux_index[i], 0, flux_index[i] < len(fluxes), flux_index[i] + 1):
                ...
                assign(freq[i], freqs_qua[flux_index[i]])
                with for_(*from_array(t[i], idle_times)):
                    ...
                    assign(phi[i], Cast.mul_fixed_by_int(detuning * 1e-9, 4 * t[i]))
                    assign(phi2[i], Cast.mul_fixed_by_int(4e-9, t[i] * freq[i]))
                    assign(phi[i], phi2[i] + phi[i])
...
if not node.parameters.simulate:
    ...
    plt.tight_layout()
    plt.show()
    node.results["figure_parabola"] = grid.fig
# %%
